...
class _backendState extends State<backend> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Column(children:[Container(child: Text("Selected Cropped Image"),) , SizedBox(width:double.infinity, height:300,child: Container( child: Image.file(File(widget.path)))),Text("Send for processing"),ElevatedButton(onPressed:(){ sendImageToFlask(widget.path).then((value) {
            if (value != '') {
      ...
      },onError: (e){showCupertinoDialog(context: context, builder: (context){return AlertDialog(
          title: Text('Error'),
...
Future sendImageToFlask(String imageFile) async {
  final response = await http.post(
    Uri.parse("http://192.168.0.100:5000/process-image"),
    headers: {
